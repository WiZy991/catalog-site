"""
Заполняет поле description у оптовых товаров из данных "Кузов" и "Двигатель" из розничных аналогов.
"""
...
class Command(BaseCommand):
    help = 'Заполняет поле description у оптовых товаров из розничных аналогов'
    ...
    def handle(self, *args, **options):
        ...
        for wholesale_product in wholesale_products:
            ...
            if retail_product.description and retail_product.description.strip():
                if wholesale_product.description != retail_product.description:
                    if dry_run:
                        self.stdout.write(
                            f'[DRY RUN] Будет обновлено описание у товара "{wholesale_product.name}" '
                            f'(ID: {wholesale_product.pk}) из розничного аналога'
                        )
                    else:
                        wholesale_product.description = retail_product.description
