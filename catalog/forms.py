...
class BulkImageUploadForm(forms.Form):
    ...
    apply_single_to_all = forms.BooleanField(
        label='Применить одно фото ко всем товарам без фотографий',
        required=False,
        initial=False,
        help_text='Если включено, первое загруженное изображение будет добавлено только в активные товары, у которых ещё нет ни одного фото. Товары с уже загруженными фото не меняются.',
    )
