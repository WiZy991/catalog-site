...
MAIN_CATEGORIES = {
    'Автоэлектрика': [
        ...
        'датчик уровня', 'датчик света салона', 'датчик движения',
        # Общее слово "датчик" - проверяется последним (низкий приоритет)
        'датчик', 'sensor',
    ],
    'Двигатель и выхлопная система': [
        ...
        'форсунка', 'тнвд', 'насос топливный', 
        # Фильтры двигателя (должны быть перед общим "фильтр")
        'фильтр топливный', 'фильтр масляный', 'фильтр воздушный',
        'топливный фильтр', 'масляный фильтр', 'воздушный фильтр',
        'масляный насос', 'помпа', 'водяной насос', 
        'термостат', 'термостаты', 'радиатор', 'вентилятор', 'вискомуфта',
        ...
        'thermostat', 'fuel filter', 'oil filter', 'air filter',
        # Генераторы и стартеры - это части двигателя
        'стартер', 'генератор', 'starter', 'generator', 'alternator',
        # Система зажигания - часть двигателя
        'катушка зажигания', 'катушка', 'свеча', 'свеча накала', 'свеча зажигания',
        'свечи накала', 'свечи зажигания', 'свечи', 'spark plug', 'glow plug',
        'sparkplug', 'glowplug', 'spark', 'glow', 'coil', 'ignition',
        # Датчики двигателя (должны быть перед общим "датчик")
        'датчик температуры', 'датчик давления масла', 'датчик давления топлива',
        'датчик кислорода', 'датчик детонации', 'датчик распредвала', 
        'датчик коленвала', 'датчик массового расхода', 'датчик положения дросселя',
        'датчик холостого хода', 'датчик температуры охлаждающей жидкости',
        'датчик температуры двигателя', 'датчик давления во впускном коллекторе',
        'датчик положения распредвала', 'датчик положения коленвала',
    ],
    'Детали подвески': [
        'амортизатор', 'пружина', 'рычаг', 'сайлентблок', 
        # Шаровые опоры и опоры подвески (не опоры двигателя)
        'шаровая опора', 'шаровая', 'опора амортизатора', 'опора стойки',
        'стойка амортизатора', 'стойка', 'втулка стабилизатора', 'стабилизатор', 
        'подшипник ступицы', 'ступица', 'кулак', 'цапфа', 
        # Тяги подвески (не тяги других систем)
        'тяга стабилизатора', 'тяга рулевая', 'наконечник рулевой',
        'наконечник', 'пыльник', 'отбойник',
        'suspension', 'shock', 'spring', 'arm', 'bushing', 'bearing', 'hub',
        'подвеск', 'подрамник', 'кронштейн подвески',
    ],
    'Трансмиссия и тормозная система': [
        ...
        # Рулевое управление (должно быть перед общим "рулевая")
        'рулевая рейка', 'рулевой механизм', 'рулевая колонка', 'рулевой вал',
        'рулевой наконечник', 'рулевая тяга', 'рулевой шарнир', 'рулевой насос',
        'гидроусилитель руля', 'электроусилитель руля', 'гур', 'эур',
        'рулевая', 'рулевое', 'steering', 'rack', 'pinion',
        # Тормозная система (должны быть перед общим "тормоз")
        'тормозная колодка', 'тормозной диск', 'тормозной суппорт',
        'тормозной цилиндр', 'тормозной шланг', 'тормозная жидкость',
        'тормоз', 'колодки', 'диск тормозной', 'суппорт', 'цилиндр тормозной',
        'шланг тормозной', 'abs', 'transmission', 'clutch', 'brake', 'gearbox',
        'трансмисс', 'привод', 'гидроблок', 'мехатроник', 'соленоид',
        'хаб', 'лок хаб', 'лок/хаб', 'крышка хаба', 'крышка электрического хаба',
        'датчик скорости колеса', 'датчик abs', 'датчик тормозной системы',
    ],
}
...
SUBCATEGORY_KEYWORDS = {
    ...
    # Двигатель и выхлопная система
    # Генераторы и стартеры - это части двигателя
    'стартер': ('Двигатель и выхлопная система', 'Стартеры'),
    'starter': ('Двигатель и выхлопная система', 'Стартеры'),
    'генератор': ('Двигатель и выхлопная система', 'Генераторы'),
    'generator': ('Двигатель и выхлопная система', 'Генераторы'),
    'alternator': ('Двигатель и выхлопная система', 'Генераторы'),
    # Система зажигания - часть двигателя
    'свеча накала': ('Двигатель и выхлопная система', 'Свечи накала'),
    'свечи накала': ('Двигатель и выхлопная система', 'Свечи накала'),
    'свеча зажигания': ('Двигатель и выхлопная система', 'Свечи зажигания'),
    'свечи зажигания': ('Двигатель и выхлопная система', 'Свечи зажигания'),
    'свеча': ('Двигатель и выхлопная система', 'Свечи зажигания'),  # По умолчанию свечи зажигания
    'spark plug': ('Двигатель и выхлопная система', 'Свечи зажигания'),
    'glow plug': ('Двигатель и выхлопная система', 'Свечи накала'),
    'катушка зажигания': ('Двигатель и выхлопная система', 'Катушки зажигания'),
    'катушка': ('Двигатель и выхлопная система', 'Катушки зажигания'),
    ...
    'лок/хаб': ('Трансмиссия и тормозная система', 'Лок/Хаб'),
    'крышка хаба': ('Трансмиссия и тормозная система', 'Крышка электрического хаба'),
    'крышка электрического хаба': ('Трансмиссия и тормозная система', 'Крышка электрического хаба'),
...
def detect_category(text):
    ...
    2. Если не найдено, использует жёстко заданные MAIN_CATEGORIES
       (проверяет более длинные/специфичные ключевые слова первыми)
    ...
    try:
        ...
        # Сортируем ключевые слова по длине (более длинные = более специфичные первыми)
        for category in db_categories:
            keywords_list = category.get_keywords_list()
            # Сортируем по длине в убывающем порядке
            sorted_keywords = sorted(keywords_list, key=lambda x: len(x), reverse=True)
    ...
    # Сортируем ключевые слова по длине для каждой категории (более специфичные первыми)
    # Сначала собираем все ключевые слова с их категориями и сортируем глобально по длине
    all_keywords = []
    for category_name, keywords in MAIN_CATEGORIES.items():
        for keyword in keywords:
            all_keywords.append((len(keyword), keyword.lower(), category_name))
    
    # Сортируем по длине в убывающем порядке (самые длинные/специфичные первыми)
    all_keywords.sort(key=lambda x: x[0], reverse=True)
    
    # Проверяем в порядке от самых специфичных к общим
    for length, keyword, category_name in all_keywords:
        if keyword in text_lower:
        ...
        for word in remaining_words:
            ...
            if (word_upper not in (result.get('article') or '').upper() and
                ...
                if not re.match(r'^\d[A-Z]{2,5}$', word_upper):
                    ...
                    # Например, "РЕМ", "ОТК", "РЕД" из "12V/80А/ПЛ. РЕМ.5Д/ОВ.Ф./ЗКОНТ" - это не отдельные характеристики
                    # Такие значения должны приходить из XML как "Размер"
                    if len(word_upper) <= 4:
                        # Пропускаем короткие слова, которые могут быть частью сложных значений
                        continue
                    # Также пропускаем слова, которые могут быть аббревиатурами из сложных значений
                    # Проверяем, есть ли в исходном названии более сложные конструкции со слешами
                    if '/' in name and word_upper in name.upper():
                        # Если слово находится в части со слешами, это может быть часть сложного значения
        ...
        if product:
            ...
            for existing_img in existing_images:
                ...
                if base_filename and stored_bn == base_filename:
                    ...
                    break
                try:
                    with existing_img.image.open('rb') as ef:
                        if hashlib.md5(ef.read()).hexdigest() == new_digest:
                            is_duplicate = True
                            break
                except Exception:
                    pass
