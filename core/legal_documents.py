...
def _strip_file_heading_duplicate(text: str) -> str:
    """
    Убирает из тела документа строки «СОГЛАСИЕ» + подзаголовок, т.к. на странице уже есть <h1>.
    """
    if not text:
        return text
    ...
    while i < len(lines) and not lines[i].strip():
        i += 1
    if i < len(lines) and 'СОГЛАСИЕ' in lines[i].upper():
        i += 1
    while i < len(lines) and not lines[i].strip():
        i += 1
    while i < len(lines):
        ...
        if not s:
            ...
            continue
        sl = s.lower()
        if sl.startswith('на ') and ('обработку' in sl or 'получение' in sl or 'рассылк' in sl):
            i += 1
            continue
        if s.startswith('(') and 'регистрац' in sl:
            i += 1
            continue
        break
    while i < len(lines) and not lines[i].strip():
...
def load_order_consent_text() -> str:
    ...
    return _strip_file_heading_duplicate(load_legal_plain_text(CONSENT_CART_FILENAME))


def load_partner_consent_text() -> str:
    """Согласие №2 — регистрация ЛК партнёра."""
    return _strip_file_heading_duplicate(load_legal_plain_text(CONSENT_PARTNER_LK_FILENAME))
